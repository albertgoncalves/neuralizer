...
const transpose = (xs) => {
    ...
    for (let iy = 0; iy < xs[0].length; iy++) {
        const y = [];
        for (let ix = 0; ix < xs.length; ix++) y.push(xs[ix][iy]);
        ys.push(y);
    }
    return ys;
};
...
const matToVecF = (f) => (xs, y) => {
    ...
    for (let i = 0; i < xs.length; i++) {
        const z = [];
        for (let j = 0; j < xs[i].length; j++) z.push(f(xs[i][j], y[i]));
        zs.push(z);
    }
    return zs;
};
...
const vecElemSumF = (f) => (x, y) => {
    let sum = 0;
    for (let i = 0; i < x.length; i++) sum += f(x[i], y[i]);
    return sum;
};
...
const matIterF = (f) => (xs) => {
    ...
    for (let ix = 0; ix < xs.length; ix++) {
        const z = [];
        for (let iy = 0; iy < xs[0].length; iy++) z.push(f(xs[ix][iy]));
        zs.push(z);
    }
    return zs;
};
